...
def AnswerOfQ_O(text, rate = 0):
	if text == "а":
	...
	elif ratio(text, "сколько"):
		tts.ospeak("Столько сколько нужно")
	elif ratio(text, "а кто тебя создал") or ratio(text, "кто тебя придумал") or ratio(text, "о разработчике"):
		tts.ospeak("Меня создал разработчик Рома, с ником The EnG1nE его можно найти в интернете по его нику")
		...
		text = "Я умею: управлять мышью и клавиатурой, и троллить человеков... Чтобы управлять мышью скажите: режим управления мышью и диктуйте сторону и количество пикселей до тысячи... Чтобы управлять клавиатурой, скажите Режим печати под диктовку... И диктуйте текст... Чтобы нажать интер скажите вот... Чтобы выйти из режимов, скажите выход, или стоп... Ещё могу включать аудио файлы на шиндовз. Если нужен весь перечень возможностей скажите помощь"
		tts.ospeak(text)



	elif ratio(text, "помощь"):
		tts.ospeak("Чтобы текст не получился слишком длинным, я разделю его на подкоманды. Вы можете вызвать: \nПомощь в стандартном режиме,\nпомощь в режиме управления мышью,\nпомощь в режиме управления клавиатурой,\nпомощь в режимах. Вызывается произнесением одноимённых команд")

	elif ratio(text, "помощь в стандартном режиме",70):
		tts.ospeak("В стандартном режиме: \"Запиши...\" для того, чтобы записать аудио в файл data.mp3 в катологе исполняемого файла, список, для диктовки списка с аудио (папка Misc), название песни (которая должна быть в папке Misc), для её воспроизведения, стоп/выключить для остоновки проигрывания композиции. Переведи... Переводит текст с Русского языка на Английский. Команда время, выполняет функции календаря. Найди/Поиск в интернете, открывают поисковой запрос google. Переключение в режимы работает только из стандартного. Вне стандартного режима можно только вернутся к стандартному.")
	
	elif ratio(text, "помощь в режиме управления мышью", 70):
		tts.ospeak("Для выхода из режима скажите \"Стоп\", \"Выход из режима\",\"Выход\". Говорите направление движения и цифру, до 1000 (технические шоколадки) \"клик\"/\"нажми\" для щелчка ЛКМ")
	
	elif ratio(text, "помощь в режиме управления клавиатурой", 70):
		tts.ospeak('Для выхода из режима скажите \"Стоп\", \"Выход из режима\",\"Выход\". "Копировать", "Вставить", "Пробел", "убери слово" и "Ввод" отвечают за одноимённые функции, остальной текст (Который не зарезервирован) будет печататься при помощи эмуляции клавиатуры, слова - команды, которые встречаются в предложениях, будут печататься ьак, как есть, если команда не была произнесена отдельно')

	elif ratio(text, "помощь в режимах", 70):
		tts.ospeak("Переходы в другие режимы из стандартного: Режим управления мышью, Режим управления клавиатурой, повторяй. В стандартный режим можно вернуться командой \"стоп\"/\"выход из режима\"")
	...
	elif ratio(text, "плавно"):
		tts.ospeak("Ну не резко же...")
	...
	elif ratio(text,"билет"):
		tts.ospeak("Билет от банка России?")
	elif ratio(text, "банк"):
		tts.ospeak("Достали вы со своими банками")
		...
		tts.ospeak("Ыыыкран)))")
	elif "обиделся" in text:
		tts.ospeak("На обиженых воду возят")
